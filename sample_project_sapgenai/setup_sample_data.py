"""
============================================================
SAMPLE PROJECT (SAP Gen AI Hub) — One-Time Setup Script
============================================================
...
What it does:
  ...
  2. Embeds a company FAQ document into HANA vector store
     using SAP Gen AI Hub embedding model

Run:
    python setup_sample_data.py

Requires:
  - .env with HANA credentials + LLM_DEPLOYMENT_ID + EMBEDDING_DEPLOYMENT_ID
  - SAP AI Core profile configured (AICORE_HOME, AICORE_PROFILE)
...
sys.path.extend([
    ...
    str(ROOT / "hana_vector_store"),   # SAP Gen AI Hub version
])
...
print("📋 Creating SAMPLE_EMPLOYEES table...")
cursor.execute("DROP TABLE SAMPLE_EMPLOYEES IF EXISTS")
cursor.execute("""
...
conn.commit()
print(f"✅ Inserted {len(employees)} employees into SAMPLE_EMPLOYEES\n")


# ── 3. Embed company FAQ via SAP Gen AI Hub ───────────────
VECTOR_TABLE = os.getenv("VECTOR_TABLE_NAME", "VECTOR_STORE")
EMBEDDING_DIM = 1536  # text-embedding-ada-002 (SAP Gen AI Hub default)

print(f"📚 Setting up vector store: {VECTOR_TABLE}")
print("   (using SAP Gen AI Hub embedding — requires EMBEDDING_DEPLOYMENT_ID)")
create_vector_table(conn, table_name=VECTOR_TABLE, dimension=EMBEDDING_DIM)
...
docs = [Document(page_content=company_faq, metadata={"source": "company_faq", "type": "text"})]
...
print(f"✅ Company FAQ embedded into {VECTOR_TABLE} via SAP Gen AI Hub\n")
